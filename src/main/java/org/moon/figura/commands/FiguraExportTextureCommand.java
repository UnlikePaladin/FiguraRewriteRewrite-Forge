...
public class FiguraExportTextureCommand {
    ...
    private static int run(CommandContext<CommandSourceStack> context, String name) {
        ...
        try {
            ...
            context.getSource().sendSuccess(new FiguraText("command.export_texture.success"), false);
            return 1;
        } catch (Exception e) {
            context.getSource().sendFailure(new FiguraText("command.export_texture.error"));
            return 0;
        }
    }
}
